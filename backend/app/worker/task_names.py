...
directly. app.worker.tasks imports model_manager, which imports whisperx
(and, through it, torch/faster_whisper/ctranslate2) - the API process
must never pull those in, so routes reference tasks by name; only the
worker process resolves and imports the real function when it executes a
job.
"""
